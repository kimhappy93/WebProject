...
## 맡은 기능
- 비회원 : 영화정보 및 뉴스기사 조회
- 회원 : 영화 평점 및 리뷰 등록, 영화정보 및 뉴스기사 조회, 회원 비밀번호 찾기
...
## 보완점 및 후기
영화 평균 평점 기능을 추가해서 평점순으로 영화 리스트를 조회하는 기능과 클라이언트가 위치해 있는 지역의 날씨를 기준으로 영화를 추천하는 기능을 구현해보고 싶습니다. 웹페이지 제작을 처음 구현해보아서 기획한대로 기능 구현하는 것을 첫 번째 목적으로 두었는데 기능을 추가해서 구현할 시간을 제작 기간에 두었다면 조금 더 완성도 있는 웹페이지를 만들 수 있지 않을까 하는 아쉬움이 있었습니다.
<br><br>
프로젝트를 진행하면서 예상치 못하게 그만둔 조원들이 있어서 각자 맡은 파트와 떠난 조원들의 파트를 같이 구현하기가 벅차기도 했지만 주석의 필요성과 남은 조원들을 한 번 더 돌아보는 계기가 되었고, 협동하여 기능을 완성해서 팀 프로젝트만의 보람을 느낄 수 있었습니다. 또한 스스로 학습하고 문제를 해결하는 능력을 기를 수 있었습니다.

## 깨달은 점
- 웹페이지 기능 추가와 보완의 기간을 두고 개발 계획을 해야 하는 점
